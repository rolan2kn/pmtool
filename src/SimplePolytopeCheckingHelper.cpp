...
bool SweepLine::intersect( SLseg* s1, SLseg* s2)
{
    ...
    double lsign, rsign;
    lsign = PointRepository::instance()->areaSign(s1->leftP, s1->rightP, s2->leftP);    // s2 left point sign
    rsign = PointRepository::instance()->areaSign(s1->leftP, s1->rightP, s2->rightP);    // s2 right point sign
    if (lsign * rsign > 0) // s2 endpoints have same sign relative to s1
        return false;      // => on same side => no intersect is possible

    lsign = PointRepository::instance()->areaSign(s2->leftP, s2->rightP, s1->leftP);    // s1 left point sign
    rsign = PointRepository::instance()->areaSign(s2->leftP, s2->rightP, s1->rightP);    // s1 right point sign
