...
TestCaseGenerator::TestCaseGenerator(const std::string& file_name, int trialsCount, int _maxPointCount)
:trialsCount(trialsCount), maxPointCount(_maxPointCount), maxValue(1000), filename(file_name)
{
}
...
void TestCaseGenerator::testSimpleRandomDistribution()
{
    PointDataGenerator pdg(maxValue);

    resultfile<<"\n ================================================================================= \n";
    resultfile <<"\nPrueba con distribucion normal de los puntos, se generara un total maximo de "<<maxPointCount;
    resultfile<<"\n ================================================================================= \n";

    resultfile<<"EL maximo valor que pueden tomar los puntos tanto en x como en y es de "<<maxValue<<"\n\n";

    resultfile<<"Se haran "<<trialsCount<<" pruebas utilizando esta distribucion variando la cantidad de puntos aleatoriamente para saber cual algoritmo se comporta mejor.\n\n"<<endl;

    ofstream csv(currentDirectory() + "/Docs/normal.csv");

    long giftTime = 0L, grahanTime = 0L;

    int timesGiftbetter = 0, timesGrahanBetter = 0, areBothEqual = 0;
    int currentSize = trialsCount;
    csv<<"Cantidad de Puntos, Gift Wrapping, Graham Scan"<<endl;

    for (int i = 0; i < trialsCount; i++)
    {
        int init = currentSize/trialsCount;
        init = (currentSize == init) ? 1 : init;
        ...
        std::vector<int> indexes2 = PointRepository::instance()->getIndexes(); // we get the point indexes


        Polygon* poly = new Polygon();          // create the polygon

        grahanTime = giftTime = 0L;
        giftTime = clock();
        poly->giftWrapping(indexes1);
        giftTime = clock2milisec(giftTime); // compute the time in milliseconds used for giftwrapping

        grahanTime = clock();
        poly->grahamScan(indexes2);
        grahanTime = clock2milisec(grahanTime); // compute the time in milliseconds used for grahanScan
        ...
        if (giftTime < grahanTime)
        {
            cout <<".... El GiftWrapping fue mejor\n"<<endl;
            ++timesGiftbetter;
        }
        else if(giftTime > grahanTime)
        {
            cout <<".... El GrahanScan fue mejor\n"<<endl;
            ++timesGrahanBetter;
        }
        else
        {
            cout <<".... Ambos fueron iguales\n"<<endl;
            ++areBothEqual;
        }

        delete poly;
        PointRepository::instance()->clear(); // remove all generated points
        csv<<pointSize<<","<<giftTime<<","<<grahanTime<<endl;
        csv.flush();
        cout.flush();

        if (i % 10 == 0)
        {
            currentSize *= trialsCount;
            if (currentSize > maxPointCount)
                currentSize = maxPointCount;
        }

    }

    float giftPercent = (timesGiftbetter * 100)/trialsCount;
    float grahanPercent = (timesGrahanBetter * 100)/trialsCount;
    float samePercent = (areBothEqual * 100)/trialsCount;

    resultfile<<endl<<"El GiftWrapping se comporto mejor en un "<< giftPercent<<" por ciento."<<endl;
    resultfile<< "El GrahanScan se comporto mejor en un "<< grahanPercent<<" por ciento."<<endl;
    resultfile<< "Ambos metodos se comportaron igual en un "<< samePercent<<" por ciento."<<endl;
    resultfile.flush();
    csv.close();
}

void TestCaseGenerator::testInHullPointDistribution()
{
    PointDataGenerator pdg(maxValue);

    resultfile<<"\n ================================================================================= \n";
    resultfile <<"Prueba con distribucion de puntos tal, que de un total random de puntos \nse selecciona un porcentaje random que pertenece a la envoltura convexa"
                 " y el resto pertenece al interior del poligono\n";

    resultfile<<"EL maximo valor que pueden tomar los puntos tanto en x como en y es de "<<maxValue<<"\n\n";

    resultfile<<"Se haran "<<trialsCount<<" pruebas utilizando esta distribucion con porciento en el hull \nvariando la cantidad de puntos aleatoriamente para saber cual algoritmo se comporta mejor\n\n"<<endl;
    resultfile<<"\n ================================================================================= \n";

    long giftTime = 0, grahanTime = 0;

    int timesGiftbetter = 0, timesGrahanBetter = 0, areBothEqual = 0;
    int currentSize = trialsCount;

    ofstream csv(currentDirectory() + "/Docs/inhull.csv");
    csv<<"Total de Puntos::Porcentaje en el hull, Gift Wrapping, Graham Scan"<<endl;
    ...
    for (int i = 0; i < trialsCount; i++)
    {
        ...
        int percent = pdg.randomInt(1, 100);
        pdg.randomPointCloudWithPercentOnHull(pointSize, percent);
        ...
        giftTime = clock();
        cout.flush();
        Polygon* pgw = poly->giftWrapping(indexes1);
        giftTime = clock2milisec(giftTime); // compute the time in milliseconds used for giftwrapping
        cout.flush();

        grahanTime = clock();
        cout.flush();
        Polygon* pgs = poly->grahamScan(indexes1);
        ...
        if (giftTime < grahanTime)
        {
            cout <<"\n.... El GiftWrapping fue mejor\n"<<endl;
            ++timesGiftbetter;
        }
        else if(giftTime > grahanTime)
        {
            cout <<"\n.... El GrahanScan fue mejor\n"<<endl;
            ++timesGrahanBetter;
        }
        else
        {
            cout <<"\n.... Ambos fueron iguales\n"<<endl;
            ++areBothEqual;
        }

        delete poly;
        PointRepository::instance()->clear(); // remove all generated points
        csv<<pointSize<<"::"<<percent<<","<<giftTime<<","<<grahanTime<<endl;
        cout.flush();
        csv.flush();
...
void TestCaseGenerator::testInsidePointDistribution()
{
    PointDataGenerator pdg(maxValue);

    resultfile<<"\n ================================================================================= \n"
                 "Prueba con distribucion de puntos tal, que de un total random de puntos \nse selecciona un porcentaje random que pertenece al interior del poligono y el resto al la envolvente convexa"<<endl;

    resultfile<<"EL maximo valor que pueden tomar los puntos tanto en x como en y es de"<<maxValue<<"\n\n";

    resultfile<<"Se haran "<<trialsCount<<" pruebas utilizando esta distribucion con porciento en el interior \nvariando la cantidad de puntos aleatoriamente para saber cual algoritmo se comporta mejor\n\n"<<endl;
    resultfile<<"\n ================================================================================= \n";

    long giftTime = 0L, grahanTime = 0L;
    int timesGiftbetter = 0, timesGrahanBetter = 0, areBothEqual = 0;

    ofstream csv(currentDirectory() + "/Docs/inside.csv");
    csv<<"Total de Puntos::Porciento en el Poligono, Gift Wrapping, Graham Scan"<<endl;
    ...
    for (int i = 0; i < trialsCount; i++)
    {
        int pointSize = pdg.randomInt(currentSize/trialsCount, currentSize);
        int percent = pdg.randomInt(1, 100);
        pdg.randomPointCloudWithPercentInside(pointSize, percent);
...
void TestCaseGenerator::testPolygon()
{
    resultfile <<"\n\n~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*\n "
                 "Prueba de la clase Poligono. "
                 "Solamente se prueban los algoritmos de la envolvente convexa"
                 "Se realizan pruebas con nubes de puntos de tres tipos: \n"
                 "\t1. Generadas aleatoriamente, con distribucion Normal.\n"
                 "\t1. Generadas aleatoriamente, con un porcentaje elegido aleatoriamente perteneciente al Hull.\n"
                 "\t1. Generadas aleatoriamente, con un porcentaje elegido aleatoriamente perteneciente al interior del poligono.\n"
                 "Se realizan "<<trialsCount<<" trials"
                                                                        "\n~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*\n"<<endl;
    this->testSimpleRandomDistribution();
    this->testInHullPointDistribution();
    this->testInsidePointDistribution();
}
