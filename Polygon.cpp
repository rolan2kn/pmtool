    ...
    do
        {
            ...
            bHA = PointRepository::instance()->area2(a1, a, b) >= 0; // leftOn
            aHB = PointRepository::instance()->area2(b1, b, a) >= 0; // leftOn
...
/**
 * We check if we have a simple polygon.
 * **/
...
//             true  = IS simple
    EventQueue  Eq(head_edge);
    SweepLine   SL(head_edge);
...
bool Polygon::isConvex()
{
    ...
    while(inLeft && i < last)
    {
        ...
        inLeft = PointRepository::instance()->area2((*pointList)[i], (*pointList)[(i+1)%last], (*pointList)[(i+2)%last]) >= 0;
        ++i;
    }
...
// iterar por todos los vertices y si forma un angulo concavo marcarlo para dividir, si queda por dividir unirlo al primero
    ...
    do {
        ...
        if (PointRepository::instance()->area2(a, b, c) < 0)    // if angle in b is concave we have a lot of work
        {
...
bool Polygon::isInCone(int i, int j, const vector<int>& P)
{
    ...
    if (PointRepository::instance()->area2(P[in1], P[i], P[i1]) >= 0) // P[i] convexo
    {
        return (PointRepository::instance()->area2(P[i], P[j], P[in1]) > 0) &&
        (PointRepository::instance()->area2(P[j], P[i], P[i1]) > 0);
    }
    return (PointRepository::instance()->area2(P[i], P[j], P[i1]) > 0) &&
           (PointRepository::instance()->area2(P[j], P[i], P[in1]) > 0);
}
...
Polygon* Polygon::grahamScan(std::vector<int> pointcloud)
{
    ...
    for (int i = 3; i < count; i++)
    {
        ...
        while (S.size() > 1 && PointRepository::instance()->area2(nextToTop(S), S.top(), points[i]) <= 0)
            S.pop();
        S.push(points[i]);
    }
...
vector<int> Polygon::auxQuickHull(std::vector<int> S, int a, int b)
{
    ...
    {
        if(PointRepository::instance()->area2(a, c, S[i]) > 0)
            A.push_back(S[i]);
        if(PointRepository::instance()->area2(c, b, S[i]) > 0)
            B.push_back(S[i]);
    }
    ...
    for(int i =0; i < size; i++)
    {
        if(PointRepository::instance()->area2(c1, a, S[i]) > 0)
            A1.push_back(S[i]);
        if(PointRepository::instance()->area2(b, c1, S[i]) > 0)
            B1.push_back(S[i]);
    }
